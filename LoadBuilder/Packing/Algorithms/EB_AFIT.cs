    ...
    public class EB_AFIT : IPackingAlgorithm
    {
        ...
        public AlgorithmPackingResult Run(Container container, List<Item> items, bool unloadingWithClamp)
        {
	        if (unloadingWithClamp)
			{
				...
				{
					if (LoadingType.IsUnloadingWithClamp(item.LoadingType))
					{
						...
						{
							Console.WriteLine($"Item quantity {item.Quantity} is not valid for clamping for item {item.ItemId}. Enter a value in multiples of {amountOnX * amountOnZ}.");
							return null;
						}
		...
		private void FindBox(decimal hmx, decimal hy, decimal hmy, decimal hz, decimal hmz)
		{
			...
			for (y = 1; y <= itemsToPackCount; y = y + itemsToPack[y].Quantity)
			{
				...
				// Item Rotation
				AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim1, itemsToPack[x].Dim2, itemsToPack[x].Dim3); // Type 0
				...
				else if (itemsToPack[x].RotationType == RotationType.Full)
				{
					...
					{
						AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim1, itemsToPack[x].Dim3, itemsToPack[x].Dim2); // Type 5
						AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim2, itemsToPack[x].Dim1, itemsToPack[x].Dim3); // Type 4
						AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim2, itemsToPack[x].Dim3, itemsToPack[x].Dim1); // Type 3
						AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim3, itemsToPack[x].Dim1, itemsToPack[x].Dim2); // Type 1
		...
		private int GetRotationType(Item item)
		{
			var rotationType = 0;
			if (item.PackDimX == item.Dim1 && item.PackDimY == item.Dim2 && item.PackDimZ == item.Dim3)
			{
				rotationType = 0;
			} 
			else if (item.PackDimX == item.Dim3 && item.PackDimY == item.Dim1 && item.PackDimZ == item.Dim2)
			{
				rotationType = 1;
			}
			else if (item.PackDimX == item.Dim3 && item.PackDimY == item.Dim2 && item.PackDimZ == item.Dim1)
			{
				rotationType = 2;
			}
			else if (item.PackDimX == item.Dim2 && item.PackDimY == item.Dim3 && item.PackDimZ == item.Dim1)
			{
				rotationType = 3;
			}
			else if (item.PackDimX == item.Dim2 && item.PackDimY == item.Dim1 && item.PackDimZ == item.Dim3)
			{
				rotationType = 4;
			}
			else if (item.PackDimX == item.Dim1 && item.PackDimY == item.Dim3 && item.PackDimZ == item.Dim2)
			{
				rotationType = 5;
			}
		...
		private void Initialize(Container container, List<Item> items)
		{
			...
			result = new ContainerPackingResult(1);
			...
			foreach (Item item in items)
			{
				...
				{
					Item newItem = new Item(item.ID, item.ItemId, item.Type, item.Dim1, item.Dim2, item.Dim3, item.RotationType, item.Quantity, item.LoadingType);
